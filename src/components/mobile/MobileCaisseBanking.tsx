...
type MobileCaisseBankingProps = {
  ...
  decaissementsHref?: string;
  montantsAttendusHref?: string;
  encheresHref?: string;
};

const ACTIONS = [
  { key: "contributions", label: "Contributions", text: "Versements enregistrés" },
  { key: "imputations", label: "Imputations", text: "Ventilation des montants" },
  { key: "decaissements", label: "Décaissements", text: "Sorties de fonds" },
  { key: "montantsAttendus", label: "Montants attendus", text: "Référentiel des restes" },
...
export default function MobileCaisseBanking({
  ...
  decaissementsHref = "/decaissements",
  ...
  const hrefByKey = {
    ...
    decaissements: decaissementsHref,
    montantsAttendus: montantsAttendusHref,
    encheres: encheresHref,
  };
      ...
      <section className="rounded-[28px] border border-emerald-100 bg-gradient-to-br from-emerald-50 via-white to-white p-6 shadow-sm">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-bold text-slate-900">
            Vue réelle de la caisse
          </h2>
          <p className="mt-3 text-sm text-slate-600">
            Lecture claire des montants, des rubriques, des mouvements récents et du total des enchères.
          </p>
        </div>
      ...
      <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-4">
          ...
          <h3 className="mt-2 text-xl font-bold text-slate-900">
            Pages qui alimentent la caisse
          </h3>
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {ACTIONS.filter((item) => item.key !== "montantsAttendus" || canAccessBureau).map((item) => (
            <Link
              ...
              className="rounded-[22px] border border-slate-200 bg-slate-50 px-4 py-4 transition hover:border-emerald-200 hover:bg-emerald-50"
            >
