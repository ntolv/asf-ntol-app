...
export async function POST(
  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Vous n'êtes pas autorisé à revenir sur une modification de décaissement.",
        },
        {
          status: 403,
        }
    ...
    const body =
      (await request.json()) as RetourArriereBody;
    ...
    const {
      ...
      await supabase.rpc(
        "fn_decaissement_revenir_arriere",
        {
          p_decaissement_id:
            decaissementId,
    ...
    console.error(
      "Erreur POST /api/decaissements/revenir-arriere:",
      error
    );
    ...
    return NextResponse.json(
      {
        ...
        message:
          error?.message ||
          "Erreur serveur lors du retour arrière.",
      },
      {
        status: 500,
      }
