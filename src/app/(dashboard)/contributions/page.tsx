  ...
  useEffect(() => {
    async function loadContributionsData() {
      try {
        ...
        // Utiliser le helper partagé avec timeout
        ...
        if (montantsError) throw montantsError;
        ...
        const totalContributions = safeContributions.reduce(
          (sum: number, c: ContributionData) => sum + (c.montant_total || 0),
          0
        );
        const totalMontantsAttente = safeMontants.reduce(
          (sum: number, m: MontantAttenteData) => sum + (m.montant_restant || 0),
          0
        );
        const totalRetards = safeRetards.reduce(
          (sum: number, r: RetardData) => sum + (r.reste_a_payer || 0),
          0
        );
      ...
      <div className="space-y-6">
        <section>
          ...
          <p className="mt-2 text-slate-600">
            Gérez vos contributions, montants en attente et retards.
          </p>
        </section>
