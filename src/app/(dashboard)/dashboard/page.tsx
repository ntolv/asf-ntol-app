  ...
  useEffect(() => {
    async function loadDashboard() {
      try {
        ...
        const { data: isAdminData, error: isAdminError } = isAdminResult;
        ...
        setBloc2(safeBloc2);
        setBloc3(safeBloc3);
        setIsCurrentUserAdmin(Boolean(isAdminData));
        setContributions(safeContributions);
        setRetards(safeRetards);
        setNotifications(safeNotifications);
      } catch (err: any) {
        console.error("Erreur dashboard:", err, err?.message, err?.details, err?.hint);
        ...
        <section className="rounded-3xl bg-gradient-to-br from-white to-green-50/30 p-6 shadow-lg ring-1 ring-green-100">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <Link
                href="/admin/roles"
                className="sm:col-span-2 inline-flex items-center justify-center rounded-2xl border border-slate-300 bg-white px-4 py-3 font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50"
              >
